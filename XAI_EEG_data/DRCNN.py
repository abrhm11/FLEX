...
class Sleep_model_MultiTarget(nn.Module):
    ...
    def forward(self, seg_fft_pos, x, seg_idx, seg_length):
        # calc seg_fft from pos and neg part
        M = seg_fft_pos.shape[0]
        seg_fft = torch.cat([
            seg_fft_pos,
            torch.conj(seg_fft_pos[1:M-1].flip(0))
        ])
        seg = torch.fft.ifft(seg_fft).real
        seg= seg.view(1, 1, seg_length)
        lower = seg_idx * seg_length
