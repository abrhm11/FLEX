...
for i, indices_i in enumerate(all_indices):
    ...
    for k in range(len(indices_i)):
        ...
        for j in range(len(indices_i_k)):
            ...
            x_segment = np.array(segments_i[j])
    ...
    bars = ax1.bar(
        all_frequencies,
        mean_amplitudes,
        color=cmap(mean_weighted_attributions / np.max(mean_weighted_attributions)),
    ...
    ax1.set_ylabel("Average Amplitudes")
    ax1.set_title(f"Average weighted attributions of frequencies in {phases[i]} stage")
    ...
    bars_pos = ax2.bar(
        ...
        color=cmap_pos(mean_weighted_pos_attributions / np.max(mean_weighted_pos_attributions)),
    ...
    bars_neg = ax2.bar(
        ...
        color=cmap_neg(mean_weighted_neg_attributions / np.max(mean_weighted_neg_attributions)),
    ...
    ax2.set_ylabel("Average Attribution")
    ax2.set_title(f"Average weighted positive/negative attributions in {phases[i]} stage")
    ax2.set_xlim(0, 40)
    ax2.set_ylim(-np.max(mean_weighted_neg_attributions) * 1.1, np.max(mean_weighted_pos_attributions) * 1.1)
